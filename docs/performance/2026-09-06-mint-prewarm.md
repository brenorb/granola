...
requests if cached metadata were accepted as sufficient evidence. It would need
another fresh check before any operation that can lock or claim proofs; otherwise
the saved request is paid back at a more dangerous boundary.
...
choose an output keyset the mint rejects. This is an availability failure when
the mint rejects before spending, not evidence that stale metadata is safe for a
financial boundary.
...
would exceed what this client verifies. A safe request reduction would therefore
need a new, explicit freshness boundary immediately before locking/claiming and
tests for active-flag rotation, mint failure and recovery. That boundary would
restore the metadata requests currently counted as critical. No such policy
